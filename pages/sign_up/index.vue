            ...
            <b-row class="" align-h="center">
                <b-col lg="9" cols="12">
                    <b-form>
                        ...
                        <b-input :class="{form_fill: user.name}" @blur="handleNameBlur" v-model.trim="user.name" :state="error_state.name" size="lg" id="login-name" placeholder="Charlie Exampleton"></b-input>
                        <b-form-invalid-feedback :state="error_state.name">
                            {{error.name}}
                        </b-form-invalid-feedback>
                        <label class="m-email" for="login-email">Email</label>
                        <b-input :class="{form_fill: user.email}" @blur="handleEmailBlur" v-model.trim="user.email" :state="error_state.email" size="lg" id="login-email" placeholder="charlie@email.com"></b-input>
                        <b-form-invalid-feedback :state="error_state.email">
                            {{error.email}}
                        </b-form-invalid-feedback>
                        <label class="m-password" for="login-password">Password</label>
                        <b-input :class="{form_fill: user.password}" @blur="handlePasswordBlur" v-model="user.password" :state="error_state.password" size="lg" id="login-password" type="password" placeholder="••••••••"></b-input>
                        <b-form-invalid-feedback :state="error_state.password">
                            {{error.password}}
                        </b-form-invalid-feedback>
                        <label class="m-password" for="confirm-password">Confirm Password</label>
                        <b-input :class="{form_fill: user.confirm_password}" @blur="handleCPasswordBlur" v-model="user.confirm_password" :state="error_state.confirm_password" size="lg" id="login-confirm_password" type="password" placeholder="••••••••"></b-input>
