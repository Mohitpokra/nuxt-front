<template>
  <b-container
    :class="!$auth.loggedIn ? 'm-container before-login' : 'm-container'"
  ...
  >
    <b-navbar type="primary" variant>
      ...
      <b-navbar-nav
        ...
        v-if="
          $route.name === 'sign_in' ||
            $route.name === 'sign_in-forgot_password' ||
            $route.name === 'sign_in-forgot_password-success'
        "
      >
        <b-nav-text v-if="!$auth.loggedIn" class="p3"
          >Need an account?</b-nav-text
        >
        <b-nav-item v-if="!$auth.loggedIn" class="p3 text-primary" to="/sign_up"
          ><span class="text-primary">Sign Up</span></b-nav-item
        >
      </b-navbar-nav>
